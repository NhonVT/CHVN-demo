...
(function (factory) {
    ...
    var Comments = {
        ...
        events: {
            ...
            // navcomment
            'click .navcomment li[data-sort-key]' : 'navcommentElementClicked',
            'click .navcomment li.title' : 'togglenavcommentDropdown',
        ...
        getDefaultOptions: function() {
            return {
                ...
                enablenavcomment: true,       
                postCommentOnEnter: false,        
                forceResponsive: false,       
                readOnly: false,      
                defaultnavcommentSortKey: 'newest',       
        ...
        init: function(options, el) {
            ...
            this.currentSortKey = this.options.defaultnavcommentSortKey;
        ...
        showActiveContainer: function() {
            var activenavcommentEl = this.$el.find('.navcomment li[data-container-name].active');
            var containerName = activenavcommentEl.data('container-name');
        ...
        updateToggleAllButton: function(parentEl) {
            ...
            if(toggleAllButton.find('span.caret-text').text() == this.options.textFormatter(this.options.hideRepliesText)) {
        ...
        showActiveSort: function() {
            var activeElements = this.$el.find('.navcomment li[data-sort-key="' + this.currentSortKey + '"]');

            // Indicate active sort
            this.$el.find('.navcomment li').removeClass('active');
            activeElements.addClass('active');

            // Update title for dropdown
            var titleEl = this.$el.find('.navcomment .title');
            if(this.currentSortKey != 'attachments') {
                titleEl.addClass('active');
                titleEl.find('header').html(activeElements.first().html());
             } else {
                var defaultDropdownEl = this.$el.find('.navcomment ul.dropdown').children().first();
        ...
        navcommentElementClicked: function(ev) {
            var navcommentEl = $(ev.currentTarget);
            var sortKey = navcommentEl.data().sortKey;
        ...
        togglenavcommentDropdown: function(ev) {
        ...
        createHTML: function() {
			...
			// navcomment bar
            if (this.options.enablenavcomment) {
                this.$el.append(this.createnavcommentElement());
                this.showActiveSort();
            }
        ...
        createnavcommentElement: function() {
            var navcommentEl = $('<ul/>', {
                'class': 'navcomment'
            });
            var navcommentWrapper = $('<div/>', {
                'class': 'navcomment-wrapper'
            });
            navcommentEl.append(navcommentWrapper);
            ...
            // Responsive navcomment
            var dropdownnavcommentWrapper = $('<div/>', {
                'class': 'navcomment-wrapper responsive'
            });
            var dropdownnavcomment = $('<ul/>', {
                'class': 'dropdown'
            });
            ...
            dropdownnavcommentWrapper.append(dropdownTitle);
            dropdownnavcommentWrapper.append(dropdownnavcomment);
            navcommentEl.append(dropdownnavcommentWrapper);


            // Populate elements
            navcommentWrapper.append(newest).append(oldest);
            dropdownnavcomment.append(newest.clone()).append(oldest.clone());

            if(this.options.enableReplying || this.options.enableUpvoting) {
                navcommentWrapper.append(popular);
                dropdownnavcomment.append(popular.clone());
            }
            if(this.options.enableAttachments) {
                navcommentWrapper.append(attachments);
                dropdownnavcommentWrapper.append(attachments.clone());
            }
        ...
        createCommentWrapperElement: function(commentModel) {
			...
			// Add report comment
			var dotComment = '<div class="dots-box"><span></span><span></span><span></span><div class="brief"><ul><li data-type="spam-cm">Báo cáo Spam</li><li data-type="hide-cm">Ẩn bình luận</li><li data-type="delete-cm">Xóa bình luận</li></ul></div></div>';
            var viewMore = '<a class="view-more-content" href="javascript:void(0)"><span>... Xem thêm</span><span>Thu gọn</span></a>';
            
            contentWrapper.append(name).append(content);
            	
			var mainBox = $('.comment-box .commenting-field.main .textarea').first().width() - 30;
			
			if(this.options.editedText == 'pop') {
				mainBox = $('.overlay-popup .commenting-field.main .textarea').width() - 30;
			}
			
			var textLength = content.html().length;
			var maxLength =  (mainBox/8)* (100/18) + 48;
			...
			wrapper.append(dotComment);
			
			// Add date comment
			if(edited == undefined) {
				actions.append(separator).append(time);
			}else {
				actions.append(separator).append(edited);
			}

			commentWrapper.append(profilePicture).append(wrapper).append(actions);
        ...
        createCssDeclarations: function() {
            ...
            // navcomment underline
            this.createCss('.jquery-comments ul.navcomment li.active:after {background: '
                + this.options.highlightColor  + ' !important;',
                +'}');

            // Dropdown active element
            this.createCss('.jquery-comments ul.navcomment ul.dropdown li.active {background: '
                + this.options.highlightColor  + ' !important;',
                +'}');
        ...
        setToggleAllButtonText: function(toggleAllButton, toggle) {
            var self = this;
            var textContainer = toggleAllButton.find('span.caret-text');
        ...
        adjustTextareaHeight: function(textarea, focus) {
           // var textareaBaseHeight = 2.2;
            //var lineHeight = 1.45;
			 var textareaBaseHeight = 1;
            var lineHeight = 1;

            var setRows = function(rows) {
                var height = textareaBaseHeight + (rows - 1) * lineHeight/2;
